...
#define BLOCKIDX get_local_size(0)
#define BLOCKIDY get_local_size(1)
#endif
...
KERNAL_PREFIX void transposeDiagonal(GLOBAL_PREFIX double *odata, GLOBAL_PREFIX double *idata, int width, int height OPENCLSHARED(double tile[TILE_DIM][TILE_DIM+1])) {
    ...
    if (width == height) {
        blockIdx_y = BLOCKIDX;
        blockIdx_x = (BLOCKIDX+BLOCKIDY)%gridDim.x;
    } else {
        int bid = BLOCKIDX + gridDim.x*BLOCKIDY;
        ...
        if (vid && nv>0) { 
            switch (integrandNo) {
                case: AREA_KERNAL
                    ...
                    break;
                case: VOLUME_KERNAL
                    gpu_volume_integrand(verts,dim,i,nv,vid,&out[i]);
                    break;
            }
        }
    }
...
KERNAL_PREFIX void functionalGradEval(GLOBAL_PREFIX double* verts, int dim, GLOBAL_PREFIX objectgpusparse_light* s,\
                      int nelements,int gradNo, grad, GLOBAL_PREFIX double* out){
    // calculate contribution from element i
    int i = GETID;
    if (i<nelements){
        int *vid = NULL; //vertex id
        ...
        if (vid && nv>0) {
            switch (gradNo) {
                case: AREA_KERNAL
                    ...
                    break;
                case: VOLUME_KERNAL
                    gpu_volume_gradient(verts,dim,i,nv,vid,out);
                    break;
            }
        }
    }
...
DEVICE_PREFIX void gpu_matrix_addtocolumn(double *elements,int nrows, int col, double scale,  double *v) {
...
DEVICE_PREFIX bool gpu_area_gradient(double *vert, int dim, int id,int nv, int *vid, double *frc) {
