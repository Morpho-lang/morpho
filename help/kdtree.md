...
Test whether a given point is present in the tree. `ismember` returns the `KDTreeNode` if the point is found, or `false` otherwise:

    var node = tree.ismember(Matrix([1,0,0]))
...
Checks if a point is a member of a k-d tree. Returns the matching `KDTreeNode`, or `false` if the point is not present.
...
Finds all points in a k-d tree that lie within a cuboidal bounding box. Returns a list of `KDTreeNode` objects. The box may be a List or Tuple of per-axis bounds, each `(min, max)` or `[min, max]`.
