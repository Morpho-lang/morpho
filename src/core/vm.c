    ...
    for (int n=nposn+1; n<nargs; n+=2) {
        ...
        if (k>=nopt) { // If we didn't find a match, we're done with optional arguments
            if (MORPHO_ISSTRING(key) &&
                MORPHO_GETSTRING(key)->obj.status==OBJECT_ISUNMANAGED) {
                // Check whether the argument looks like an optional argument, but we just don't recognize it - the test for this is whether this is a string and it's one generated by the runtime
